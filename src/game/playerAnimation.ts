...
export const PLAYER_ANIMATION_ASSETS: PlayerAnimationAssetSpec[] = [
  { action: 'idle', directory: 'idle', filePrefix: 'player_idle-', frameCount: 4, paddedIndex: false },
  { action: 'run', directory: 'run', filePrefix: 'player_run_', frameCount: 4, paddedIndex: true },
...
export function registerPlayerAnimations(scene: Phaser.Scene): void {
  ...
  const create = (
    action: PlayerAction,
    frameCount: number,
    options: { frameRate?: number; duration?: number; repeat: number; hideOnComplete?: boolean }
  ): void => {
  ...
  create('idle', 4, { frameRate: 8, repeat: -1 });
  create('run', 4, { frameRate: 12, repeat: -1 });
  create('dash', 6, { duration: PLAYER_DASH_ANIMATION_DURATION_MS, repeat: 0 });
  create('attack-light', 5, { duration: 200, repeat: 0, hideOnComplete: true });
  create('attack-hard', 5, { duration: 200, repeat: 0, hideOnComplete: true });
