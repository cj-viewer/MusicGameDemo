...
def write_generated_ts(
    ...
    for item in exports:
        with Image.open(objects_dir / item.source.file) as prop_image:
            alpha_bounds = prop_image.convert("RGBA").getchannel("A").getbbox()
        visible_bottom = alpha_bounds[3] if alpha_bounds else item.height
        rows.append(
            "  { "
            f"id: {item.source.id}, file: '{item.source.file}', x: {item.x}, y: {item.y}, "
            f"width: {item.width}, height: {item.height}, visibleBottom: {visible_bottom} "
            "},"
        )
    rows.extend((
        "];",
...
def main() -> None:
    ...
    write_generated_ts(args.generated_ts, exports, args.objects_dir)
